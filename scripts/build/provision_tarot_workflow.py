...
DEFAULT_DSL_PATH = REPO_ROOT / "services" / "dify-agent" / "dsl" / "askcrystal-tarot-workflow-2026-04-26.dsl.yml"
WORKFLOW_TOOL_NAME = "workflow_tarot_spread_interpretation"
WORKFLOW_TOOL_LABEL = "Tarot Spread Interpretation"
WORKFLOW_TOOL_DESCRIPTION = (
    "Chooses a tarot spread, generates or normalizes the draw, and returns a structured spread contract for the main AskCrystal agent to interpret."
)
...
WORKFLOW_TOOL_PARAMETERS = [
    {
        "name": "question",
        "description": "The user's tarot question or reading theme.",
        "form": "llm",
    },
    {
        "name": "spread_type",
        "description": "Optional spread hint such as single, three, diamond, moon, horseshoe, or celtic.",
        "form": "llm",
    },
    {
        "name": "cards_drawn",
        "description": "Optional user-provided cards if the reading should interpret an existing spread instead of drawing fresh cards.",
        "form": "llm",
    },
    {
        "name": "context_anchor",
        "description": "Optional recent concrete detail that should ground the reading. For fresh draws, include AskCrystal runtime local time and timezone when available.",
        "form": "llm",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import, publish, and register the AskCrystal tarot workflow as a Dify workflow tool."
    )
    ...
    parser.add_argument("--publish-marked-name", default="Tarot workflow", help="Release marker name shown in Dify workflow history.")
