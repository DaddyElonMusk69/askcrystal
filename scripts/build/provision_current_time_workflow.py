...
DEFAULT_DSL_PATH = REPO_ROOT / "services" / "dify-agent" / "dsl" / "askcrystal-current-time-workflow-2026-04-28.dsl.yml"
WORKFLOW_TOOL_NAME = "current_time"
WORKFLOW_TOOL_LABEL = "Current Time"
WORKFLOW_TOOL_DESCRIPTION = (
    "Returns the current local date, time, timezone, UTC offset, UTC timestamp, and relative "
    "yesterday/today/tomorrow anchors for time-sensitive AskCrystal readings."
)
...
WORKFLOW_TOOL_PARAMETERS = [
    {
        "name": "timezone",
        "description": (
            "Optional IANA timezone such as Asia/Shanghai or America/Los_Angeles. "
            "Use the user's browser/profile timezone when available. Defaults to Asia/Shanghai."
        ),
        "form": "llm",
    },
    {
        "name": "format",
        "description": "Optional Python strftime format. Defaults to %Y-%m-%d %H:%M:%S %Z.",
        "form": "llm",
    },
    {
        "name": "locale",
        "description": "Optional user locale, included only as metadata for downstream formatting context.",
        "form": "llm",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import, publish, and register the AskCrystal current-time workflow as a Dify workflow tool."
    )
    ...
    parser.add_argument("--publish-marked-name", default="Current time", help="Release marker name shown in Dify workflow history.")
