...
2. `Interactive agent UI layer`
   ...
   - Homepage app block renders the agent-led storefront stage
   - React island mounted inside the app block assets
...
- `React` for the homepage agent island,
...
- we should not rebuild drawers, dialogs, tabs, scroll containers, and popovers from scratch,
...
2. `Agent app block`
   - mounted inside the homepage template
   - renders the conversation stage and dynamic curation surface
   - loads its own JS/CSS from theme app extension assets
...
1. Build a Shopify app with:
   - React Router template
   - theme app extension
   - app proxy
   - inside `apps/shopify/`

2. Put the homepage experience into:
   - a homepage app block inside the theme extension
...
`Shopify OS 2.0 theme + Theme App Extension homepage block + App Proxy backend + Dify orchestration`

For the UI implementation inside that homepage block:
