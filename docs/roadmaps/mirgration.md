...
- Keep Shopify theme, Shopify app, Dify agent assets, and Dify runtime as separate units.
- Host the Shopify theme on Shopify.
- Host the Shopify app/proxy on our own cloud/server.
- Host Dify on our own cloud/server.
...
#### Phase 6: Retired Skill Bridge

- Stop: not applicable
- Reason: the old bridge runtime has been removed and replaced by workflow-native Dify tools
...
- [x] Replace the old bridge runtime with Dify-native workflow tools
- [x] Remove `services/skill-bridge`
- [x] Remove bridge start/register/sync scripts
...
| `dify_skill_bridge` | removed | Retired | Replaced by Dify-native workflow tools |
...
- Whether `external_skills/` should stay as external source material or be reduced after workflow parity is stable
