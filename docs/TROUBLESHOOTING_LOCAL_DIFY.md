...
- `scripts/start_skill_bridge.sh` now prefers an existing runtime with `fastapi/uvicorn/pydantic` and starts without forcing pip install.
- If you want auto-install to `.venv`, set:

```bash
SKILL_BRIDGE_AUTO_INSTALL=1 ./scripts/start_skill_bridge.sh
```
