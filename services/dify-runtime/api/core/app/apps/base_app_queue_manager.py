...
class AppQueueManager(ABC):
        ...
        try:
            while True:
                try:
                    ...
                    if elapsed_time >= listen_timeout or self.is_stopped():
    ...
    def is_stopped(self) -> bool:
        """
        Check if task is stopped without relying on the short-lived cache.
        This is used on the active cancellation path so user stop requests
    ...
    def raise_if_stopped(self) -> None:
        """
        Raise the shared stop exception as soon as a stop flag is observed.
        :return:
        """
        if self.is_stopped():
