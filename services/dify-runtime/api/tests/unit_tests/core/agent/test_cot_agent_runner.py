...
def runner(mocker):
    ...
    runner.raise_if_stopped = MagicMock()
...
class TestHandleInvokeAction:
    ...
    def test_handle_invoke_action_honors_stop_requests(self, runner):
        action = AgentScratchpadUnit.Action(action_name="tool", action_input={})
        runner.raise_if_stopped.side_effect = GenerateTaskStoppedError()

        with pytest.raises(GenerateTaskStoppedError):
            runner._handle_invoke_action(action, {"tool": MagicMock()}, [])
...
class TestRun:
    def test_run_honors_stop_requests(self, runner):
        runner.raise_if_stopped.side_effect = GenerateTaskStoppedError()

        with pytest.raises(GenerateTaskStoppedError):
            list(runner.run(MagicMock(id="msg-id"), "query", {}))
