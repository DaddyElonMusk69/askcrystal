...
```bash
python3 scripts/build/provision_storefront_component_flow.py
```

Smoke-test the proxy component stream after starting the Shopify proxy with `DIFY_APP_ID` set to the provisioned app id:

```bash
python3 scripts/dev/smoke_storefront_component_proxy.py --proxy-base-url http://localhost:8787
```
