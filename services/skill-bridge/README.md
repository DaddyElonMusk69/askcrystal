...
pip install -r services/skill-bridge/requirements.txt
PYTHONPATH=services/skill-bridge/src uvicorn dify_skill_bridge.server:app --host 0.0.0.0 --port 8010
```
...
Add new files under `services/skill-bridge/src/dify_skill_bridge/skills/*.yml` (or `.json`):
